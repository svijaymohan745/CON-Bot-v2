...
exports.run = (client, message, args, sql, Discord) =>{
  ...
  var crLevel = {command: "rlevel", description: "Add/Remove roles that users gain at a certain level. Make sure that there is a role named **CON client** (only people with this role can add roles).\n\nTo **add** a role use: \`\`_rlevel add level RoleName\`\` \nTo **remove** a role use: \`\`_rlevel remove RoleName\`\`"};
