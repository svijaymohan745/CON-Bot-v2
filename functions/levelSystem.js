...
module.exports.scoreSystem = function(client, message, sql, Discord){
  sql.get(`SELECT * FROM userScores WHERE guildID = '${message.guild.id}' AND userID = '${message.author.id}'`).then(row =>{
    ...
    if (!row){
      ...
      let curPoints = row.globalPoints + 1.5;
      ...
      sql.run(`UPDATE userScores SET username='` + eUsername +`', globalPoints = ${row.globalPoints + 7}, weeklyPoints = ${row.weeklyPoints + 7} WHERE userID = ${message.author.id} AND guildID = ${message.guild.id}`);//updates points
